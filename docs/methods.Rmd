...
- Used a linear model and Tukey Honest Significant Differences to test for differences between pipelines and across sequencing runs for each of the diversity metrics calculated for the raw count data. 
    - Equation: $\bar{D_i} = P_j\Beta_0 + S_k\Beta_1 + P*S\Beta_2$ 
        - $D_{ijk}$ mean diversity metric for sample $i$, pipeline $j$, and sequencing run $k$   
        - $P_j$ - bioinformatic pipeline   
        - $S_k$ - sequencing run  
    - R: `lm(mean_dist~pipe*seq_run_id, data = dat)`
- Evaluated the impact of normalization methods for NIST run 1.  
- Used a linear model to and TukeyHSD to quantify the impact of different normalization methods on sample to sample variability. 
    - Indendent models fit for each diversity metric-pipeline combination. 
    - R: `lm(mean_dist~normalization, data = dat)`
