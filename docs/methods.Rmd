...
The source code for all statistical analysis is available at https://github.com/nate-d-olson/diversity_assessment.
...
The unnormalized count data was used to characterize the baseline beta-diversity repeatability for the different pipeline and sequencng runs.
A linear model was used to quantify differences between pipelines and across sequencing runs for each of the diversity metrics.
Data from sequencing run NIST1 was used to evaluate the impact of normalization methods on beta-diversity repeatability.
...
Next, we evaluated the signal-to-noise ratio for the different pipelines across sequencing runs by comparing pre-exposure samples to the titration and post-exposure samples from the same subject.
Signal was measured as the median beta-diversity between PCRs from the samples being compared (Fig. \@ref(fig:experimentalDesign)).
Noise was measured as the median between PCR replicate beta-diversity.
A weighted average of the signal-to-noise ratio was calculated as the area under the curve (using the `trapz` function) of the signal-to-noise ratio and the proportion of pre-exposure sample in the sample being compared [@pracma].
Tndependent linear models were fit for each diversity metric to quantify differences in the signal-to-noise ratio between sequencing runs and pipelines.
A mixed effects linear model was then used to quantify normalization methods impact on the signal-to-noise ratio using data from NIST1 with subject as a random effect.
...
To quantify the contribution of biological and technical variability to total variability the distribution of beta diversity metrics were compared between individuals, within individual between conditions (pre- and post-exposure), and different types of technical replicates.
A linear model was then used to quantify differences in beta diversity between biological and technical sources of variability.
...
Distance-based redundancy analysis (dbRDA) was used to test for significant sources of variation [@vegan].
