...
* Similar to PCR beta diversity repeatability we evaluated the how well normalization methods are able to account for differences in the sample total abundance by comparing the unnormalized to normalized count data signal to noise ration for the first NIST run (Fig. \@ref(fig:sigalToNoiseNormEffectPlot)).
* Weighted metrics 
    + For most methods normalization methods did not have a significant impact on signal to noise. 
    + TSS increased signal to noise
    + UQ, RLE, and TMM decreased signal to noise relative
