...
auc_summary %>%
    ungroup() %>%
    mutate(seq_lab_run = paste0(seq_lab,"_run", seq_run)) %>%
...
Signal to noise ratio for unweighted metrics around 1 for all pipelines and sequencing runs (Fig. \@ref(fig:sigalToNoiseRawPlot)).
Signal to noise ratios around 1 indicate that the magnitude of the signal (biological differences) was comparable to the noise (differences between PCR replicates).
Signal to noise ratio was defined as the beta diversity between unmixed pre-exposure samples and the other samples in the titration series (signal) divided by the beta diversity between PCR replicates within the samples being compared.
Only DADA2 and mothur signal to noise ratios were greater than the data pre-processed using the QIIME pipeline.
DADA2 and Mothur higher signal to noise ratio differences for JHU runs compared to NIST runs, especially for weighted phylogenetic.
The relationship between NIST and JHU runs for the signal to noise relationship is consistent with the PCR replicate beta diversity repeatabiltiy results.

Similar to PCR beta diversity repeatability we evaluated the how well normalization methods account for differences in the sample total abundance by comparing the first NIST run signal to noise ratio for un-normalized to normalized count data  (Fig. \@ref(fig:sigalToNoiseNormEffectPlot)).
Normalizing count data should increase the signal to noise ratio.
Most normalization methods did not have a significant effect on the signal to noise ratio for weighted metrics (Fig. \@ref(fig:sigalToNoiseNormEffectPlot)A).
TSS significantly increased the Bray-Curtis signal to noise ratio for Mothur and DADA2, but not for the other pipelines or weighted UniFrac.
Some normalization methods significantly decreased the signal to noise ratio for weighted beta diversity metrics.
Rarefying counts to 15th quantile resulted in significantly lower weighted Unifrac and Bray-Curtis Beta-diversity signal to noise for QIIME closed-reference and QIIME _de novo_.
While RLE and TMM improved PCR replicate beta diversity, they also significantly lowered the weighted UniFrac beta diversity for DADA2, Mothur, and QIIME _de novo_.
While, rareifying count data often increased signal-to-noise ratio for unweighted metrics the increase was only significant for DADA2 and Mothur.
Rarefying count data to 2000 and 5000 significantly improved the DADA2 signal-to-noise ratio for both unweighted metrics.
Mothur signal-to-noise ratio was only significantly higher for Jaccard and when count data was rareifed to 2000. 
