...
Signal-to-noise ratio for unweighted metrics was around 1 for all pipelines and sequencing runs (Fig. \@ref(fig:sigalToNoiseRawPlot)).
Signal-to-noise ratios around 1 indicate that the signal magnitude (biological differences) was comparable to the noise (differences between PCR replicates).
We define the signal-to-noise ratio as the beta diversity between unmixed pre-exposure samples and the other samples in the titration series (signal) divided by PCR replicate beta diversity for the samples being compared.
Only DADA2 and mothur signal to noise ratios were greater than the pipelines pre-processed using QIIME.
...
Similar to PCR beta diversity repeatability we evaluated the how well normalization methods account for differences in the sample total abundance by comparing un-normalized to normalized NIST run 1 signal-to-noise ratio (Fig. \@ref(fig:signalToNoiseNormEffectPlot)).
Normalizing count data should increase the signal-to-noise ratio.
Most normalization methods did not have a significant effect on the signal-to-noise ratio for weighted metrics (Fig. \@ref(fig:signalToNoiseNormEffectPlot)A).
...
Rarefying counts to 15th quantile resulted in significantly lower weighted UniFrac and Bray-Curtis beta-diversity for QIIME closed-reference and _de novo_.
While RLE and TMM improved PCR replicate beta diversity, these normalization methods also significantly lowered the weighted UniFrac beta diversity for DADA2, Mothur, and QIIME _de novo_.
While, rareifying count data often increased unweighted metric signal-to-noise ratio though the increase was only significant for DADA2 and Mothur.
