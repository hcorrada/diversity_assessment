...
We calculated a signal-to-noise ratio by dividing the beta diversity between unmixed pre-exposure samples and samples in the titration series (signal) by PCR replicate beta diversity for the samples being compared.
...
however, most normalization methods did not have a significant effect when using weighted metrics (Fig. \@ref(fig:signalToNoiseNormEffectPlot)A).
...
Rarefying counts to the 15th quantile resulted in significantly lower weighted UniFrac and Bray-Curtis beta-diversity for QIIME closed-reference and _de novo_ pipelines.
...
Rarefaction often increased unweighted metric signal-to-noise ratio (Fig. \@ref(fig:signalToNoiseNormEffectPlot)B), though the increase was only significant at lower subsampling depths for DADA2 and Mothur pipelines.
