...
weighted <- ggplot(varpart_weighted) +
    geom_point(aes(x=normalization, y=Adj.R.square, 
                   group=feature, fill=feature, color=feature), 
               size=2, position=position_dodge(width = 0.8), shape=16)+
    geom_point(aes(x=normalization, y=Raw.Adj.R.square, 
                   group=feature, fill=feature, color=feature), 
               size=2, position=position_dodge(width = 0.8), shape=17)+
    ...
    theme_bw() +
    theme(axis.text.x = element_text(angle = -45, hjust = 0)) +
    xlab("") + ylab(paste("Adjusted", expression(R^2))) +
