...
weighted <- bio_v_tech_dat %>% filter(weight == "Weighted" ) %>% 
...
unweighted <- bio_v_tech_dat %>% filter(weight == "Unweighted" ) %>% 
    ggplot(aes(x = variation_label, y = value, 
               color = variation, fill = variation)) + 
    geom_tufteboxplot() + 
    facet_grid(f_type~pipe, scales = "free_y") + 
...
ggarrange(weighted + rremove("xlab") + rremove("x.text"), 
          unweighted, 
          ncol = 1, nrow = 2, 
          labels = "AUTO", common.legend = TRUE, legend = "bottom", heights = c(0.35,0.65))
```
