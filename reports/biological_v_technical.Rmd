...
run_colors=c(jhu1="#E41A1C", jhu2="#4A72A6", nist1="#48A462", nist2="#A35390")
...
```{r bioVtechPlot0, warning=FALSE, message=FALSE, echo = FALSE, fig.cap = "Heatmaps indicating mean beta diversity of un-normalized PCR replicates sequenced at the same lab (J1-J2 or N1-N2) or at different labs (J[1-2]-N[1-2]) for each pipline and metric.", fig.height=4, fig.width=8}

replicate_variation <- biol_v_tech_variation[[3]] %>% 
    filter(same_pcr_product==TRUE) 

replicate_variation$replicate_samples<-gsub(replicate_variation$sample_a, pattern="[a-z]*_run[0-9]_", replacement="")
replicate_variation$label<-paste0(replicate_variation$sample_a, replicate_variation$sample_b)
replicate_variation$label<-gsub(replicate_variation$label, pattern="_[0-9]-[A-Z][0-9]*", replacement = "-")
replicate_variation$label<-gsub(replicate_variation$label, pattern="-$", replacement = "")
replicate_variation$label<-gsub(replicate_variation$label, pattern="jhu_run", replacement = "J")
replicate_variation$label<-gsub(replicate_variation$label, pattern="nist_run", replacement = "N")
replicate_variation$label<-factor(replicate_variation$label, 
                                  levels= c("J1-J2", 
                                  "N1-N2","J1-N1",
                                  "J1-N2", "J2-N1", "J2-N2"), ordered = T)
        ...
        mutate(pipe = if_else(pipe == "deblur", "q_deblur", 
                          if_else(pipe == "qiimeOpenRef", "q_open", 
                                  if_else(pipe == "qiimeClosedRef", "q_closed",
                                          ...
                                          pipe)))),
           pipe = factor(pipe)) %>% 
    select(pipe, metric, label, value, normalization) %>% 
    group_by(pipe, metric, label, normalization) %>% 
    summarize(mean_val=mean(value)) %>%
    filter(normalization=="RAW") %>%
    ggplot() + 
    geom_tile(aes(x = label, y = pipe, fill = mean_val)) + theme_bw() + 
    facet_grid(~metric)+
    theme(axis.text.x = element_text(angle = -45, hjust = 0),
          legend.position = "bottom") + scale_fill_distiller(palette = "Blues")+ 
        labs(x = "Replicate Comparison", y = "Pipeline", fill = "Mean Pairwise Diversity")
...
```{r bioVtechPlot2, warning=FALSE, message=FALSE, echo = FALSE, fig.cap = "Biological vs. Technical Variation, y-axis is the adjusted R^2 value, indicating proportion of variance explained by each biological (subject and titration) and technical (seq run) variable. Black bars represent values for un-normalized data.", eval=F}
...
# ggplot() +
    ...
    # theme(legend.position = "bottom")+scale_fill_manual(values = norm_colors)
