...
weighted <- bio_v_tech_dat %>%
    ...
    scale_color_manual(values=c("#a6bddb", "#3690c0", "#034e7b", "#fa9fb5", "#ae017e", "#7a0177"))+
    scale_fill_manual(values=c("#a6bddb", "#3690c0", "#034e7b", "#fa9fb5", "#ae017e", "#7a0177"))
...
```{r bioVtechVarpart, warning=FALSE, message=FALSE, echo = FALSE, fig.cap = "Biological vs. Technical Variation, y-axis is the adjusted $R^2$ value, indicating the proportion of variance explained by each biological (subject and titration) and technical (seq run) variable. Black bars represent values for unnormalized data.", fig.width = 6, fig.height = 8}
...
weighted <- ggplot(varpart_weighted) +
    geom_bar(aes(x = normalization, y = Adj.R.square, fill = feature),
             stat = "identity") +
    ...
    scale_fill_manual(values=c("#7a0177", "#a6bddb", "#3690c0"))
...
unweighted <- ggplot(varpart_unweighted) +
    geom_bar(aes(x = normalization, y = Adj.R.square, fill = feature),
             stat = "identity") +
    facet_grid(metric~pipe, scales = "free_x", space = "free") +
    theme_bw() +
