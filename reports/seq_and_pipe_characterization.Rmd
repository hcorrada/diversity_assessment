...
The four replicate sequencing runs were of variable sequence quality and depth (Fig. \@ref(fig:seqCharPlot), _Supplemental DADA2 qual plot_).
Greater variability in feature total abundance between samples was observed on the first run at each laboratory (Fig. \@ref(fig:seqCharPlot)A).
...
Sequence data from the two NIST runs had higher quality scores than the data from JHU runs, with the exception of JHU2 forward reads (Fig. \@ref(fig:seqCharPlot)C).


Overall, sequences from JHU1 had lower read quality and higher variability in total sample abundance.
Sequences from NIST run 1 were of higher quality, but also exhibited greater variability in total sample abundance.
...
Similarly, we can use data from the NIST1 to determine how well normalization methods are able to account for differences in total abundance between samples.


Samples from the different sequencing runs were processed by six different bioinformatic pipelines.
...
Aside form the four QIIME pipelines each pipeline employs its own pre-processing, clustering, and quality filtering methods.
...
We generated rarefaction curves to assess feature diversity at multiple sampling depths for across the four sequencing runs \@ref(fig:rarePlot)).
