...
cov_plot <- ggplot(fit_est_df) + 
    geom_pointrange(aes(x = seq_run_id, y = est, ymin = lci, ymax = uci)) + 
...
```{r seqCharPlot, fig.cap = "A. Distribution in the number of reads per barcoded sample (Library Size) by sequncing laboratory and sequencing run. Negative control library size is indicated by hollow points. Negative controls were not included in the boxplots. B. Coeffient of variation in total abundance by sequencing run estimate and 95\\% confidence interval obtined using a mixed effects linear model. C. PhiX error rate relative to 16S rRNA amplicon base position for initial and the four sequencing runs. D. Distribution of mode read quality score by sequencing run. "}
ggarrange(n_read_plot, cov_plot, error_plot, seq_qual_plot, labels = "AUTO")
```
