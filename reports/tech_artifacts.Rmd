...
We also expected that the second NIST run, which had higher read quality and lower total abundance variability, to have comparable, if not lower, beta diversity between PCR replicates. This was not the case.  Beta diversity was consistently higher for both the NIST runs compared to the high quality, low variability second JHU run. Furthermore, the second NIST run values were more similar to the first high quality and high total abundance variability NIST run, than the second JHU run.
...
For unweighted metrics (Fig. \@ref(fig:techNormImpactWeighted)B), rarefying count data to 2000 total abundance resulted in the lowest beta diversity between PCR replicates.
While rarefying counts to the total abundance of the 85th most abundant sample (rareq15) resulted in no differences or lowered unweighted beta diversity repeatability for most pipeline-diversity metric combinations, it resulted in higher weighed beta diversity for QIIME clustering techniques.
Rarefaction to this level is also most susceptible to sample loss and should be interpreted with caution (Supplemental Table X).
