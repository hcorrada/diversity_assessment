...
```{r techTotalAbuCOV, fig.cap = "Distribution of the coefficienct of variation of total abundance for PCR replicates by bioinformatic pipeline and sequencing run across."}
ggplot(tech_info_df) + 
    geom_boxplot(aes(x = seq_run_id, y = cov_total_abu)) + 
    labs(x = "Sequencing Run", y = "COV(Total Abundance)") + facet_wrap(~pipe) + theme_bw()
```

```{r techQual, fig.cap = "Mean read quality score for PCR replicates across sequencing runs for forward and reverse reads."}
tech_info_df %>% 
...
```{r techNormImpactJ1, fig.cap = "JHU1 - Impact of normalization method on mean beta diversity between pcr repliates."}
norm_impact %>% 
    filter(seq_run_id == "jhu_run1") %>% 
    ggplot() + 
    geom_hline(aes(yintercept = 0)) + 
    geom_point(aes(x = pipe, y = dist_change, color = normalization)) + 
...
```{r techNormImpactJ2, fig.cap = "JHU2 - Impact of normalization method on mean beta diversity between pcr repliates."}
norm_impact %>% 
    filter(seq_run_id == "jhu_run2") %>% 
    ggplot() + 
    geom_hline(aes(yintercept = 0)) + 
    geom_point(aes(x = pipe, y = dist_change, color = normalization)) + 
    facet_grid(f_type ~ weight, scales = "free") + 
...
```{r techNormImpactN1, fig.cap = "NIST1 - Impact of normalization method on mean beta diversity between pcr repliates."}
norm_impact %>% 
    filter(seq_run_id == "nist_run1") %>% 
    ggplot() + 
    geom_hline(aes(yintercept = 0)) + 
    geom_point(aes(x = pipe, y = dist_change, color = normalization)) + 
    facet_grid(f_type ~ weight, scales = "free") + 
...
```{r techNormImpactN2, fig.cap = "NIST2 - Impact of normalization method on mean beta diversity between pcr repliates."}
norm_impact %>% 
    filter(seq_run_id == "nist_run2") %>% 
    ggplot() + 
    geom_hline(aes(yintercept = 0)) + 
    geom_point(aes(x = pipe, y = dist_change, color = normalization)) + 
    facet_grid(f_type ~ weight, scales = "free") + 
...
* JHU run 1 had lower sequence quality (boxplots) (Fig. \@ref(fig:techQual)).  
* NIST run 1 had greater variabiltiy in sample total abundance (boxplots) (Fig. \@ref(fig:techTotalAbuCOV)).  
