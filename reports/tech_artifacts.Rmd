...
```{r techArtSetup, warning=FALSE, message=FALSE, echo = FALSE, cache=FALSE}
...
norm_level_ord <- c("RAW", "rare2000", "rare5000", "rare10000", "rereq15", 
                    "CSS", "TMM", "CLR", "WRE", "TSS")
...
tech_art_df_for_plot <- tech_art_df %>% ungroup() %>%
    filter(normalization %in% norm_level_ord) %>%
    filter(normalization != "RAW", seq_run_id == "nist_run1")
