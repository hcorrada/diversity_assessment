...
pipe_colors=c(dada="#5D995D", deblur="#bf548c", mothur="#ef8a1e", 
...
tech_art_df %>% 
    ...
    ggplot() + 
    geom_tufteboxplot(aes(x = normalization, y = mean_dist, color = pipe,
                          group=interaction(normalization,pipe)), position = position_dodge()) +  
    facet_grid(f_type~weight,scales = "free", space = "free") + 
    theme_bw() + 
    ...
    labs(x = "Normalization", y = "Change in Mean Beta Diversity", 
         color = "Pipeline") +scale_color_manual(values=pipe_colors)
```
