...
tech_art_df %>% filter(normalization == "RAW") %>% 
    ...
    labs(x = "Bioinformatic Pipeline", y = "Mean Distance", fill = "Sequncing Run")
```
...
tech_art_df %>% 
    ...
    labs(x = "Pipeline", y = "Change in Mean Beta Diversity", color = "Normalization") 
```

__Key Points__  

* JHU run 1 had lower sequence quality (boxplots) and greater variability in sample total abundance for QIIME pipelines (Fig. \@ref(fig:techTotalAbuCOV)).  
* NIST run 1 had greater variabiltiy in sample total abundance (lineplot) (Fig. \@ref(fig:techTotalAbuCOV)). 
* Pairwise distances varies by metric and was consistently lower for weighted unifrac compared to others (Fig. \@ref(fig:techMeanDist)).
...
* DADA2 pairwise distances greater for NIST1 and NIST2 (which had greater variability in library size) compared to JHU runs, JHU2 had the lowest pairwise distance (better quality than JHU1).  
* Mothur and dada, consistent results for JHU runs, suggesting they are better able to account for sequencing errors than other pipelines. 
* Deblur pipeline failed for JHU1, suggesting it doesn't do well handling sequencing errors.  
