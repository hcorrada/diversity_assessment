...
pipe_colors=c(dada="#5D995D", deblur="#bf548c", mother="#ef8a1e", 
              qiimeClosedRef="#f2ae02", qiimeOpenRef="#c63f4a", 
              qiimeDeNovo="#7190c4")
...
cov_plot<-ggplot(tech_info_df) +
    ...
    labs(x = "Pipeline", y = "COV(Total Abundance)") + theme_bw()+coord_flip()+
    theme(axis.text.x=element_text(angle = 45, hjust = 1))+scale_fill_manual(values=run_colors)
...
read_qual_plot<-tech_info_df %>% 
    ...
    theme_bw()+theme(axis.text.x=element_text(angle = 45, hjust = 1))+scale_fill_manual(values=c("black", "darkgray"))
...
tech_art_df %>% filter(normalization == "RAW") %>% 
    ...
    labs(x = "Bioinformatic Pipeline", y = "Mean Distance", fill = "Sequncing Run")+scale_fill_manual(values=run_colors)
```

```{r techNormImpactALL, fig.cap = "Impact of normalization method on mean beta diversity between pcr replicates, for sequencing run with higher quality and total abundance variability, NIST run 1. Data are presented as minimal-ink boxplots, where points indicate median value, gap between point and lines the interquartile range, and lines the boxplot whiskers."}
...
tech_art_df %>% 
    ...
    ggplot() + 
    geom_tufteboxplot(aes(x = normalization, y = mean_dist, color = normalization), size=0.75) + 
    facet_grid(pipe~metric,scales = "free", space = "free") + 
    theme_bw() + 
    theme(legend.position = "none", axis.text.x = element_text(angle = 90)) + 
    labs(x = "Pipeline", y = "Change in Mean Beta Diversity", color = "Normalization") +scale_color_manual(values=norm_colors)
```
