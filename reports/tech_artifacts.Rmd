...
tech_info_df$cov_total_abu=factor()

cov_plot<-ggplot(tech_info_df) +
    geom_boxplot(aes(x = reorder(label, cov_total_abu, FUN=median), 
                     y = cov_total_abu, fill=seq_run_id)) +
    labs(x = "Sequencing Run", y = "COV(Total Abundance)") + theme_bw()+coord_flip()+
